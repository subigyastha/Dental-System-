...
**Status:** normative production target, grounded in the Prisma schema at `prisma/schema.prisma` as reviewed on 2026-08-15. The detailed complexity/performance audit is in `docs/21-database-complexity-and-performance-review.md`.
...
All instants are exchanged as UTC ISO 8601 values and represented by Prisma `DateTime`. The current PostgreSQL columns are `timestamp(3) without time zone`, so the API must continue to normalize them as UTC until a rehearsed `timestamptz` migration is approved. Gregorian/AD is canonical. BS is a presentation conversion only; never persist a BS date as the authoritative value. Store local schedule rules as IANA timezone + local time (`HH:mm`), normally `Asia/Kathmandu`.
...
|---|---|---|
| `Organization` | Tenant: `id`, name, business type, contact details, `status`, timezone, `primaryCalendar`, timestamps. | Has locations, users, providers, clients, services, appointments, finance, workflow/audit data. Indexed by status and business type. `primaryCalendar` defaults to AD with optional BS display. |
| `Location` | Organization site: name, address/phone, timezone, `isActive`. | Belongs to Organization; currently referenced by resources, availability, blocks, provider services, appointments. Indexed `(organizationId,isActive)`; the redundant organization-only index was removed. |
...
| `User` | Current staff/account record: profile, credential hash, employment metadata, schedulability, lifecycle `status`, and compatibility scalar `role`. | Optional Organization (permits SuperAdmin); optional 1:1 Provider. Additive OrganizationMembership and RoleAssignment records are implemented, including race-safe active-role uniqueness. The scalar role remains a temporary compatibility source until cutover. |
...
| `Customer` (legacy Client) | Client demographic/clinical-alert data: name, compatibility phone/email, normalized contact values, DOB, address, emergency contact, clinical alerts, `patientCode`, and last visit. | Shared household phones are allowed. `ClientPhone` is the authoritative multi-value contact table; compatibility rows are backfilled and seed-safe. Active-directory partial and trigram indexes support browse/search. `patientCode` is organization-unique when supplied. |
...
| `Appointment` | Provider booking: organization/client/provider/location, UTC `startsAt`/`endsAt`, duration/buffer, status, priority, notes, cancellation, recurrence link. | Provider overlap is prevented at commit time by a PostgreSQL exclusion constraint. Query indexes cover organization/provider/date, organization/client/date, organization/status/date, organization/date, and location/date. Interval/duration/buffer checks are database-enforced. |
...
| `FollowUpTask` | Client/appointment task: type, status, priority, due date, summary/action, provider owner. | Tenant/client scoped; optional appointment/provider. Query indexes align with dashboard `(organization,status,dueAt)`, Client timeline `(organization,client,dueAt)`, appointment, and owner lookups. |
...
| `Invoice` | Finance document: organization/client/appointment/location, idempotent create/issue metadata, number, status, timestamps, totals/balance/notes. | Unique tenant invoice number and command idempotency keys; database amount checks; query indexes for tenant/location/status/date and Client/date. Issued invoices are protected by governed commands, while the legacy required `issuedAt` on drafts remains a compatibility debt. |
...
| `Payment` | Collection receipt: invoice/client/appointment/location, amount/currency/method/status/reference/date, recorder and idempotency metadata. | Positive NPR amount, tenant/location/date and relation indexes, and governed correction links are implemented. Completed receipts reject generic edit/delete paths. |
...
Planned scheduling-settings refinement: add `OrganizationSetting.slotStartIntervalMinutes` with default `15` and an allowlist of `5|10|15|20|30|60`. This is an Owner-controlled presentation/candidate-start rule, not appointment duration or capacity. The migration must backfill existing organizations to 15, add a database check, audit changes, and increment/invalidate the schedule configuration version without rewriting appointments.
