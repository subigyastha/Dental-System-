...
| `/billing/invoices` and nested payments | Legacy GETs are compatibility-only and location-scoped; legacy mutations return `410 Gone`. | `/v1/finance/*`; idempotent named commands and append-only completed payments/corrections. Provider intents/webhooks remain P6B. |
...
| Clients: `GET/POST /clients`, `GET/PATCH /clients/:id`, `POST /clients/:id/phones`, `POST /clients/:id/caller-phone`, `POST /clients/:id/archive`, `POST /clients/:id/delete-requests`, `POST /client-deletion-requests/:id/confirm`, `POST /clients/:id/merge`, `POST /clients/matches` | Clinic operators R/W by capability. Providers may create through governed match review and use `caller-phone` only with a current candidate-set version proving the selected Client; the generic phone endpoint stays administrative. Providers cannot generally correct profiles, archive, merge, delete, or resolve identity reviews. Client identifiers are immutable/aliased; common phones are allowed. Only Owner confirms final deletion after archive and blockers check. |
...
Implemented P4B v1 routes (all under `/api`, cookie session + CSRF for commands, and the standard v1 envelope):

- `GET /v1/finance/workspace?locationId&clientId` — bounded invoice rows, gross collections, executed corrections, balance summary, and server capabilities.
- `POST /v1/finance/invoices` — idempotent location-scoped draft creation using decimal-string NPR line amounts.
- `GET /v1/finance/invoices/:id` — frozen line snapshot and immutable payment/correction ledger.
- `POST /v1/finance/invoices/:id/issue` — idempotent Finance/Owner/Admin issuance.
- `POST /v1/finance/invoices/:id/payments` — idempotent eligible payment recording; Receptionist allowed, and non-cash reference required.
- `POST /v1/finance/invoices/:invoiceId/payments/:paymentId/corrections` — idempotent Refund/Reversal request with remaining-correctable enforcement.
- `GET /v1/finance/corrections?status=PendingApproval`, `POST /v1/finance/corrections/:id/approve`, and `/reject` — versioned distinct Owner/Admin decision workflow.
- `GET /v1/finance/reconciliation?date&locationId` — read-only Nepal-day totals, method breakdown, pending approvals, and deterministic exceptions.

Implemented scheduling startup read model:

- `GET /v1/schedule/bootstrap?detail=summary|full` — authenticated, tenant/location-scoped active Provider references and active services. `summary` omits availability and block collections while retaining Provider service IDs needed by edit/reschedule, and is used by Reservations; `full` is the default compatibility contract and is retained for the My Schedule editor. Both modes intentionally exclude Clients, staff, Records, follow-ups, and appointment history. Providers may view colleagues here; write/booking commands continue to enforce Provider-self restrictions independently.
- Visible legacy appointment range reads temporarily include minimal Client/provider/service labels so Schedule does not preload a Client directory. Full Client search remains demand-driven through `/v1/clients`; this range compatibility endpoint is still scheduled for a versioned `/v1/schedule/appointments` replacement.
